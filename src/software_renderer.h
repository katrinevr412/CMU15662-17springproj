...
typedef Matrix3x3 mtx3;

class SoftwareRenderer : public SVGRenderer {
 public:
 //int test;
  SoftwareRenderer( ) : sample_rate (1) { }
  //const SampleMethod Image_Rasterizing_Method = NEAREST;
  // Free used resources
...
class SoftwareRendererImp : public SoftwareRenderer {
 public:
  ...
  unsigned char *wei_target;
  int wei_target_h, wei_target_w;
  
  std::stack<mtx3> transStk;

	SoftwareRendererImp( ) : SoftwareRenderer( ), wei_target(NULL) { 
		while(!transStk.empty()) transStk.pop();
		//transStk.push(transformation);
		//std::cout << "initial transformation: " << transformation << std::endl;
	}
 ...
 private:
  ...
  // rasterize a point
  void rasterize_point( float x, float y, Color color, bool tag );
  ...
  void rasterize_triangle( float x0, float y0,
	...
	void rasterize_ellipse(Vector2D center, Vector2D radius, Color cBound, Color cIn);

	bool inside_ellipse(Vector2D testPoint, Vector2D center, Vector2D radius);
